...
export const Navbar: React.FC = () => {
 ...
 return (
  <header className="sticky top-0 z-40 w-full border-b border-ink-700/80 bg-surface-canvas/80 backdrop-blur-xl transition-all" suppressHydrationWarning>
    ...
    <div className="flex items-center justify-between h-16 gap-4">
        ...
        <button
         ...
         className={`relative flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold transition-all ${
          active
           ...
           : 'text-ink-500 hover:text-ink-300 hover:bg-surface-canvas/60'
         }`}
        >
         ...
         {link.count !== undefined && link.count > 0 && (
          <span
           className="ml-0.5 px-1.5 py-0.2 rounded-full text-xs font-mono bg-orange-600/30 text-orange-300"
     ...
     <div className="flex items-center gap-2 md:gap-3 flex-1 max-w-xs md:max-w-md justify-end">
       ...
       <ClientOnly>
        {session?.user ? (
         <button
          ...
          className="flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-semibold text-ink-300 hover:text-white hover:bg-ink-700 transition-colors cursor-pointer group"
         ...
         >
          {session.user.avatarUrl ? (
           ...
           <UserIcon className="w-4 h-4 text-ink-500 group-hover:text-white" />
          )}
      ...
      <button
       ...
       className="p-2 rounded-xl text-ink-500 hover:text-surface-primary hover:bg-ink-700 md:hidden transition-colors"
   ...
   {mobileMenuOpen && (
    <div className="md:hidden border-b border-ink-700 bg-surface-canvas/95 backdrop-blur-2xl px-4 py-4 space-y-3 animate-in fade-in slide-in-from-top-3 duration-200">
     <div className="flex items-center justify-between px-2 py-1 border-b border-ink-700 pb-3 gap-2">
      <div className="flex flex-col gap-1">
       <span className="text-xs text-ink-500 font-medium">Data Source:</span>
       <ClientOnly>
        <DataSourceSelector compact />
       </ClientOnly>
      </div>
      <div className="flex flex-col items-end gap-1">
       <span className="text-xs text-ink-500 font-medium">Title Lang:</span>
       <ClientOnly>
        <TitleLanguageToggle />
       </ClientOnly>
      </div>
