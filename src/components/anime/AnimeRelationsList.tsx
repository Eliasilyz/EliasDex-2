    ...
    return (
      <div className="space-y-3 animate-pulse">
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className="h-20 bg-zinc-900/60 rounded-2xl border border-zinc-800" />
        ))}
      </div>
    );
  }
  ...
  if (!relations || relations.length === 0) {
    return (
      <div className="text-center py-12 px-4 rounded-2xl bg-zinc-900/30 border border-zinc-800/80 space-y-2">
        <GitFork className="w-8 h-8 text-zinc-500 mx-auto" />
        <p className="text-sm font-semibold text-zinc-300">No related entries found</p>
        <p className="text-xs text-zinc-500">This anime is a standalone title or relations are not cataloged.</p>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      <div className="space-y-3">
        {relations.map((relGroup, gIdx) => (
          <div
            key={gIdx}
            className="bg-zinc-900/60 border border-zinc-800/80 rounded-2xl p-4 space-y-3 shadow-sm"
          >
              ...
              {relGroup.entry.map((entry) => {
                ...
                if (isAnime) {
                  return (
                    <div
                      ...
                      className="group flex items-center justify-between gap-3 p-3 rounded-xl bg-zinc-800/60 hover:bg-zinc-800 border border-zinc-750/70 hover:border-orange-500/50 transition-all cursor-pointer"
                    >
                      <div className="flex items-center gap-2.5 min-w-0">
                        ...
                        <div className="min-w-0">
                          <h4 className="text-xs font-bold text-zinc-200 group-hover:text-orange-400 transition-colors truncate">
                            {entry.name}
                          </h4>
                      ...
                      <div className="p-1.5 rounded-lg bg-zinc-700/50 group-hover:bg-orange-500 text-zinc-400 group-hover:text-white transition-colors shrink-0">
                ...
                return (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="group flex items-center justify-between gap-3 p-3 rounded-xl bg-zinc-800/40 hover:bg-zinc-800/80 border border-zinc-750/50 hover:border-zinc-600 transition-all"
                  >
                    <div className="flex items-center gap-2.5 min-w-0">
                      ...
                      <div className="min-w-0">
                        <h4 className="text-xs font-semibold text-zinc-300 group-hover:text-purple-300 transition-colors truncate">
                          {entry.name}
                        </h4>
