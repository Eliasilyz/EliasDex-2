...
export const ThemeSongsList: React.FC<ThemeSongsListProps> = ({
 ...
 return (
  ...
  {Array.from({ length: 4 }).map((_, i) => (
   <div key={i} className="h-16 bg-surface-canvas/60 rounded-2xl border border-ink-700" />
  ))}
  </div>
 );
 }
 ...
 <div className="space-y-4">
  ...
  <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 pb-2 border-b border-ink-700/60">
  <div className="flex items-center gap-1.5 p-1 bg-surface-canvas/80 rounded-xl border border-ink-700 text-xs">
   <button
   type="button"
   id="themes-tab-op"
  ...
  <div className="flex items-center gap-2">
   ...
   <button
    ...
    className="px-3 py-1.5 rounded-xl bg-orange-600 hover:bg-orange-500 text-white text-xs font-bold flex items-center gap-1.5 shadow-md transition-all cursor-pointer"
   >
  ...
  {/* Song Track Rows */}
  {activeList.length === 0 ? (
  <div className="text-center py-10 px-4 rounded-2xl bg-surface-canvas/30 border border-ink-700/80 space-y-2">
   <Music className="w-8 h-8 text-ink-500 mx-auto" />
  ...
  <div className="space-y-2.5">
   ...
   return (
    ...
    className={`group relative border rounded-2xl p-3 sm:p-3.5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 transition-all duration-200 shadow-sm ${
     isThisTrackActive
     ? 'bg-orange-950/20 border-orange-500/80 ring-1 '
     : 'bg-surface-canvas/60 hover:bg-surface-raised/90 border-ink-700/80 hover:border-ink-500'
    }`}
    >
    ...
    <div className="flex items-center gap-3 min-w-0 flex-1">
     ...
     <div className="flex flex-wrap items-center gap-2">
      ...
      className={`text-sm font-bold truncate cursor-pointer transition-colors ${
       isThisTrackActive ? 'text-orange-400' : 'text-surface-primary group-hover:text-orange-400'
      }`}
      >
      {parsed.title}
      </h4>
      {parsed.episodes && (
      <span className="px-2 py-0.5 rounded text-xs font-medium bg-ink-700 text-ink-300 border border-border-subtle shrink-0">
       {parsed.episodes}
      </span>
      )}
     </div>
     <p className="text-xs text-ink-500 flex items-center gap-1.5">
      ...
      <span className="font-medium text-ink-300">{parsed.artist}</span>
     </p>
     </div>
    </div>

    {/* Right Action Tools (Play, Watch MV, Copy, YouTube, Spotify) */}
    <div className="flex items-center gap-2 self-end sm:self-center shrink-0 pt-2 sm:pt-0 border-t sm:border-t-0 border-ink-700/60 w-full sm:w-auto justify-end">
     ...
     className="p-1.5 sm:p-2 rounded-xl bg-ink-700/80 hover:bg-ink-500 text-ink-300 hover:text-white text-xs inline-flex items-center gap-1.5 border border-ink-500/60 transition-colors cursor-pointer"
     >
     {copiedIndex === trackId ? (
     ...
     <span className="hidden sm:inline text-xs">YT</span>
     </a>
     ...
     <span className="hidden sm:inline text-xs">Spotify</span>
     </a>
    </div>
    </div>
   );
