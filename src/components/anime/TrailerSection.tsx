...
export const TrailerSection: React.FC<TrailerSectionProps> = ({
  ...
  if (!embedUrl && !youtubeId && !watchUrl) {
    return (
      <div className="text-center py-12 px-4 rounded-2xl bg-zinc-900/30 border border-zinc-800/80 space-y-2">
        ...
        <p className="text-sm font-semibold text-zinc-300">No official trailer available</p>
        <p className="text-xs text-zinc-500">Official promotional video has not been linked for this anime.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      {/* Video Container */}
      <div className="relative w-full aspect-video rounded-2xl sm:rounded-3xl overflow-hidden bg-black border border-zinc-800 shadow-2xl group">
        {isPlaying && embedUrl ? (
          <iframe
          ...
          <div className="relative w-full h-full">
            ...
            {thumbnailUrl ? (
              <img
                ...
                className="w-full h-full object-cover object-center filter brightness-90 group-hover:scale-102 transition-transform duration-500"
              />
            ) : (
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-3">
              <button
                ...
                className="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-orange-500 hover:bg-orange-400 text-white flex items-center justify-center shadow-xl shadow-orange-500/30 hover:scale-110 active:scale-95 transition-all duration-200 cursor-pointer group/btn"
              ...
              <div className="text-center px-4">
                ...
                <span className="text-[11px] sm:text-xs text-zinc-300 drop-shadow">
      ...
      <div className="flex items-center justify-between gap-3 text-xs text-zinc-400 px-1">
        <div className="flex items-center gap-2">
          {isPlaying ? (
            ...
            <span className="text-zinc-500">HD Streaming Quality via YouTube</span>
          )}
        </div>
        ...
        {watchUrl && (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-1.5 text-zinc-300 hover:text-orange-400 font-medium transition-colors"
          >
