...
export const AnimeCard: React.FC<AnimeCardProps> = ({
 ...
 return (
  <div
   ...
   className={`group relative flex flex-col h-full rounded-2xl bg-surface-canvas/50 border border-ink-700/60 hover:border-ink-500/80 hover:shadow-xl hover:shadow-black/40 transition-all duration-300 overflow-hidden ${className}`}
  >
   {/* Poster Image Box */}
   <div
    onClick={handleClick}
   ...
   <div className="p-3 flex flex-col flex-1 justify-between gap-2">
    <div className="space-y-1">
     <div className="flex items-start justify-between gap-2">
      <h3
       onClick={handleClick}
       className="text-[13px] sm:text-sm font-semibold text-surface-primary hover:text-orange-400 transition-colors line-clamp-2 cursor-pointer leading-snug flex-1 min-h-[2.4rem] flex items-start"
       title={title}
      >
       {title}
      </h3>
      ...
      {showWatchlistQuickBtn && (
       <div className="relative shrink-0">
        ...
        {showStatusMenu && (
         <div
          className="absolute right-0 bottom-full mb-2 w-36 bg-surface-canvas border border-ink-500 rounded-xl shadow-2xl py-1 z-30 text-xs animate-in fade-in duration-150"
          onClick={(e) => e.stopPropagation()}
         >
          <button
           type="button"
           onClick={(e) => handleStatusChange('watching', e)}
           className="w-full px-3 py-1.5 text-left hover:bg-ink-700 flex items-center justify-between text-ink-300 cursor-pointer"
          >
           <span>Watching</span>
          ...
          {status && (
           <button
            ...
            className="w-full px-3 py-1.5 text-left hover:bg-rose-950 text-rose-400 border-t border-ink-700 mt-1 cursor-pointer"
           >
            Remove
           </button>
          )}
    ...
    <div className="flex items-center justify-between text-xs text-ink-500 pt-2 border-t border-ink-700/60">
