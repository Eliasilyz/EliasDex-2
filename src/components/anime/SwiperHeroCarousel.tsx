...
export const SwiperHeroCarousel: React.FC<SwiperHeroCarouselProps> = ({
  ...
  return (
    <div className="relative w-full rounded-3xl overflow-hidden group shadow-xl border border-ink-700/60 bg-surface-canvas">
    <Swiper
     ...
     >
      {items.map((anime) => {
       const bgImage =
        ...
        '';
       const displayTitle = getTitle(anime);
      ...
      <SwiperSlide key={anime.mal_id} className="relative w-full h-full overflow-hidden">
       ...
       <div className="relative z-10 h-full max-w-4xl flex flex-col justify-end p-6 sm:p-10 md:p-12 space-y-4">
        ...
        <h1
         ...
         className="text-2xl sm:text-4xl md:text-5xl font-extrabold font-heading text-surface-primary tracking-tight leading-tight hover:text-orange-400 cursor-pointer transition-colors line-clamp-2"
        >
         {displayTitle}
        </h1>

         {anime.synopsis && (
          <p className="text-xs sm:text-sm text-ink-300 line-clamp-2 max-w-prose font-sans leading-relaxed drop-shadow">
