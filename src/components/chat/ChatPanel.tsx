...
export const ChatPanel: React.FC<ChatPanelProps> = ({
  ...
  if (!session?.user) {
    return (
      <div className="flex items-center justify-center h-64 rounded-2xl border border-dashed border-zinc-700/60 bg-zinc-900/40 p-4 text-center">
        <p className="text-sm text-zinc-500">Sign in to participate in chat</p>
      </div>
    );
  }
  ...
  return (
    <div className={`flex flex-col h-full rounded-2xl border border-zinc-800 bg-zinc-900/60 overflow-hidden ${className}`}>
      {/* Messages Container */}
      <div
      ...
      {/* Input Section */}
      <div className="border-t border-zinc-800 p-3 space-y-2 bg-zinc-900/40">
        {error && (
          <p className="text-xs text-rose-400">{error}</p>
        )}

        {isGuest ? (
          <p className="text-xs text-zinc-500 text-center">Guests can view but not send messages</p>
        ) : (
          <form onSubmit={handleSendMessage} className="flex gap-2">
            <input
              ...
              disabled={isSending}
              className="flex-1 px-3 py-2 text-xs bg-zinc-800 border border-zinc-700 rounded-lg text-white placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-orange-500 disabled:opacity-50"
            />
            <button
              type="submit"
