...
export const TitleLanguageToggle: React.FC<TitleLanguageToggleProps> = ({
    ...
    return (
      <button
        ...
        className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-xl text-xs font-bold font-mono transition-all border cursor-pointer focus:outline-none focus:ring-2 focus:ring-orange-500/40 ${
          titleLanguage === 'en'
            ? 'bg-zinc-900 border-zinc-750 text-orange-400 hover:border-orange-500/50'
            : 'bg-zinc-900 border-zinc-750 text-orange-400 hover:border-orange-500/50'
        } ${className}`}
  ...
  return (
    <div
      ...
      className={`inline-flex items-center p-0.5 rounded-xl bg-zinc-900/90 border border-zinc-800 text-xs font-semibold select-none ${className}`}
      role="group"
