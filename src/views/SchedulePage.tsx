    ...
    <div className="space-y-8 pb-16">
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-400">Discover when new episodes air across the week</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pb-1">
        {DAYS.map((day) => {
          ...
          return (
            ...
            >
              <span>{day.label}</span>
              {isToday && <span className="ml-1.5 opacity-75 font-normal text-[11px]">(Today)</span>}
            </button>
          );
        })}
      </div>
      ...
      <section className="space-y-4">
        <div className="text-xs text-zinc-400">
          Showing anime broadcasting on <span className="text-white font-semibold capitalize">{selectedDay}</span>
        </div>
