  ...
  <div className="space-y-8 pb-16">
   ...
   <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div className="flex items-center gap-2.5">
     ...
     <div>
      ...
      <p className="text-xs text-ink-500">Track and organize your personal anime library</p>
     </div>
    </div>
   </div>
     ...
     return (
      ...
      >
       ...
       <span className={`px-1.5 py-0.2 rounded-full text-xs font-mono ${
        isSelected ? 'bg-orange-700 text-white' : 'bg-ink-700 text-ink-300'
       }`}>
        {tab.count}
       </span>
      </button>
   ...
   {filteredItems.length > 0 ? (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3.5 sm:gap-4 lg:gap-5">
     {filteredItems.map((item) => {
      ...
      return (
       <div
        key={item.malId}
         className="group relative flex flex-col h-full rounded-2xl bg-surface-canvas border border-ink-700/80 hover:border-orange-500/50 hover:shadow-lg transition-all duration-300 overflow-hidden"
       >
        {/* Poster */}
        <div
         onClick={() => onNavigate(`/anime/${item.malId}`)}
         className="relative aspect-[3/4] w-full overflow-hidden bg-surface-canvas cursor-pointer"
        >
          <img
           src={item.image}
           alt={item.title}
         ...
         <div className="absolute inset-0 bg-gradient-to-t from-surface-canvas/80 via-transparent to-transparent opacity-80 pointer-events-none" />
          ...
          <button
           ...
           className="w-12 h-12 rounded-full bg-orange-600 hover:bg-orange-500 text-white flex items-center justify-center shadow-lg scale-90 group-hover:scale-100 transition-all cursor-pointer"
         ...
         <div className="absolute top-2.5 left-2.5 z-10 pointer-events-none">
           <span className="inline-flex items-center px-2 py-0.5 rounded-md text-xs font-mono uppercase font-bold tracking-wider bg-surface-canvas text-orange-400 border border-orange-500/30">
         ...
         {item.score && (
          <div className="absolute top-2.5 right-2.5 z-10 pointer-events-none">
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-xs font-bold bg-surface-canvas text-amber-500 border border-amber-500/30">
         ...
         {item.lastWatchedEpisode && (
          <div className="absolute bottom-2.5 right-2.5 z-10 pointer-events-none">
            <span className="px-2 py-0.5 rounded-md bg-orange-950 border border-orange-500/50 text-orange-400 font-bold font-mono text-xs">
        ...
        <div className="p-3 flex flex-col flex-1 justify-between gap-2">
         <div className="space-y-1">
          <h3
           onClick={() => onNavigate(`/anime/${item.malId}`)}
           className="text-[13px] sm:text-sm font-semibold text-surface-primary hover:text-orange-400 transition-colors line-clamp-2 cursor-pointer leading-snug min-h-[2.4rem] flex items-start"
           title={item.title}
          >
           {item.title}
          </h3>
          <p className="text-xs text-ink-500 truncate h-4">
           {item.type || 'Anime'}
          </p>
         </div>
         ...
         <div className="flex items-center justify-between pt-2 border-t border-ink-700/60 text-xs">
          <button
           ...
           className="text-orange-400 hover:text-orange-300 text-xs font-semibold flex items-center gap-1 cursor-pointer"
          >
          ...
          <button
           ...
           className="p-1 rounded-md text-ink-500 hover:text-rose-400 transition-colors cursor-pointer"
    ...
    <div className="py-20 text-center space-y-3 max-w-sm mx-auto">
     <Bookmark className="w-10 h-10 text-ink-500 mx-auto" />
     <h3 className="text-base font-semibold text-ink-300">No Anime in this list</h3>
     <p className="text-xs text-ink-500">Browse anime and click bookmark to save your favorite titles.</p>
