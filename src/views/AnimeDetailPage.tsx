 ...
 if (error || !anime) {
  return (
   <div className="py-20 text-center space-y-4 max-w-md mx-auto">
    <h2 className="text-xl font-bold font-heading text-surface-primary">Anime Not Found</h2>
    <p className="text-sm text-ink-500">{error || 'Could not retrieve anime information.'}</p>
    ...
    <div className="relative z-10 p-4 sm:p-7 flex flex-col md:flex-row gap-5 sm:gap-7 items-start">
     ...
     <div className="w-36 sm:w-48 md:w-56 shrink-0 mx-auto md:mx-0 flex flex-col gap-3">
      ...
      <div className="hidden md:flex flex-col gap-2 w-full">
       ...
       <div className="w-full relative">
        <div className="flex items-center justify-between px-3 py-2 rounded-xl bg-surface-canvas/90 border border-border-subtle text-xs text-ink-300">
         <div className="flex items-center gap-1.5 min-w-0">
          <Bookmark className={`w-3.5 h-3.5 shrink-0 ${watchlistStatus ? 'text-orange-400 fill-orange-400' : 'text-ink-500'}`} />
     ...
     <div className="flex-1 space-y-3.5 min-w-0 text-left">
      ...
      <div className="flex flex-wrap items-center gap-1.5 text-xs">
       ...
       <span className="px-2 py-0.5 rounded font-bold text-xs bg-amber-500/20 text-amber-300 border border-amber-500/30">
        HD
       </span>
       ...
       <span className="px-2 py-0.5 rounded font-bold text-xs bg-blue-500/20 text-blue-300 border border-blue-500/30">
        DUB: {totalEpCount}
       </span>
       ...
       {anime.score && (
        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded font-bold text-xs bg-orange-500/20 text-orange-300 border border-orange-500/30">
      ...
      <div className="flex md:hidden flex-wrap items-center gap-2 pt-1">
       ...
       <div className="relative">
        <Button
         variant="outline"
         size="sm"
         icon={<Bookmark className={`w-3.5 h-3.5 ${watchlistStatus ? 'text-orange-400 fill-orange-400' : 'text-ink-500'}`} />}
        >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-y-2 gap-x-4 text-xs pt-3 border-t border-ink-700/80 text-ink-500">
       <div>
        <span className="text-ink-500 block text-xs">Type:</span>
        <span className="font-medium text-ink-300">{anime.type || 'TV'}</span>
       </div>
       <div>
        <span className="text-ink-500 block text-xs">Studios:</span>
        <span className="font-medium text-ink-300">
         {anime.studios?.map((s) => s.name).join(', ') || 'N/A'}
        </span>
       </div>
       <div>
        <span className="text-ink-500 block text-xs">Date Aired:</span>
        <span className="font-medium text-ink-300">{anime.aired?.string || 'N/A'}</span>
       </div>
       <div>
        <span className="text-ink-500 block text-xs">Status:</span>
        <span className="font-medium text-ink-300">{anime.status || 'Finished Airing'}</span>
       </div>
       <div>
        <span className="text-ink-500 block text-xs">Duration:</span>
        <span className="font-medium text-ink-300">{anime.duration || '24m/ep'}</span>
       </div>
       <div>
        <span className="text-ink-500 block text-xs">Premiered:</span>
        <span className="font-medium text-ink-300 capitalize">
      ...
      {anime.genres && anime.genres.length > 0 && (
       <div className="flex flex-wrap items-center gap-1.5 pt-2">
        <span className="text-xs text-ink-500 font-medium mr-1">Genres:</span>
        {anime.genres.map((g) => (
         <span
          key={g.mal_id}
          onClick={() => onNavigate(`/browse?genre=${g.mal_id}`)}
          className="px-2.5 py-0.5 rounded-md text-xs font-medium bg-surface-canvas/90 hover:bg-orange-600 hover:text-white text-ink-300 border border-ink-700 cursor-pointer transition-colors"
         >
          {g.name}
         </span>
        ))}
      ...
      <div className="space-y-2 text-ink-300">
       {anime.title_japanese && (
        <div className="flex justify-between gap-2">
         <span className="text-ink-500">Japanese:</span>
         <span className="text-right font-medium text-ink-300 truncate max-w-[180px]">
          {anime.title_japanese}
         </span>
        </div>
       )}
       ...
       {anime.title_synonyms && anime.title_synonyms.length > 0 && (
        <div className="flex justify-between gap-2">
         <span className="text-ink-500">Synonyms:</span>
         <span className="text-right font-medium text-ink-300 truncate max-w-[180px]">
       ...
       <div className="flex justify-between gap-2">
        ...
        <span className="font-semibold text-ink-300">{anime.type || 'TV'}</span>
       </div>

       <div className="flex justify-between gap-2">
        <span className="text-ink-500">Episodes:</span>
        <span className="font-semibold text-ink-300">{anime.episodes || 'Unknown'}</span>
       </div>

       <div className="flex justify-between gap-2">
        <span className="text-ink-500">Status:</span>
        <span className="font-semibold text-ink-300">{anime.status || 'Finished'}</span>
       </div>

       <div className="flex justify-between gap-2">
        <span className="text-ink-500">Aired:</span>
        <span className="font-medium text-ink-300 text-right">{anime.aired?.string || 'N/A'}</span>
       </div>
       ...
       <div className="flex justify-between gap-2">
        ...
        <span className="font-medium text-ink-300">{anime.duration || '24 min'}</span>
       </div>
       ...
       <div className="flex justify-between gap-2">
        ...
        <span className="font-medium text-ink-300">{anime.rank ? `#${anime.rank}` : 'N/A'}</span>
       </div>

       <div className="flex justify-between gap-2">
        <span className="text-ink-500">Popularity:</span>
        <span className="font-medium text-ink-300">
       ...
       {anime.studios && anime.studios.length > 0 && (
        <div className="flex justify-between gap-2 pt-1 border-t border-ink-700">
         <span className="text-ink-500">Studios:</span>
       ...
       {anime.producers && anime.producers.length > 0 && (
        <div className="flex justify-between gap-2">
         <span className="text-ink-500">Producers:</span>
         <span className="font-medium text-ink-300 text-right truncate max-w-[180px]">
