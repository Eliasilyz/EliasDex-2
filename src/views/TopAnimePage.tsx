  ...
  <div className="space-y-8 pb-16">
   ...
   <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div className="flex items-center gap-2.5">
     ...
     <div>
      ...
      <p className="text-xs text-ink-500">Discover highest ranked and most celebrated series</p>
     </div>
    </div>
   </div>
   ...
   <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pb-1">
    {tabs.map((tab) => {
     ...
     return (
      ...
      >
       <Icon className={`w-3.5 h-3.5 ${isSelected ? 'text-amber-300' : 'text-ink-500'}`} />
       <span>{tab.label}</span>
      </button>
     );
    })}
   ...
   <section className="space-y-4">
    <div className="text-xs text-ink-500">
     Showing <span className="text-surface-primary font-semibold capitalize">{filter}</span> (Page {currentPage})
    </div>
