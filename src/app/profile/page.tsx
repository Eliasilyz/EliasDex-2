  ...
  return (
    <div className="space-y-6 pb-16 max-w-4xl mx-auto">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold font-heading text-white">Profile</h1>
          <p className="text-sm text-zinc-400 mt-1">Your watching stats and account settings</p>
        </div>
        <ShadcnButton
          variant="outline"
          size="sm"
            ...
            <div className="flex-1 min-w-0">
              {isEditing ? (
                <input
                  ...
                  className="w-full px-3 py-1.5 bg-zinc-800 border border-zinc-700 rounded-lg text-white text-lg font-semibold focus:outline-none focus:ring-2 focus:ring-orange-500"
              ...
              <p className="text-sm text-zinc-400 mt-0.5">{user?.email || session.user.email}</p>
            </div>
          </div>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between text-sm">
              ...
              <span className="text-zinc-300 font-mono" aria-label={`${xpIntoLevel} of ${xpNeeded} XP`}>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {favorites.slice(0, 8).map((fav) => (
                <div
                  ...
                  className="group cursor-pointer rounded-xl overflow-hidden bg-zinc-800/50 border border-zinc-700/50 hover:border-orange-500/50 transition-colors focus-visible:ring-2 focus-visible:ring-orange-500"
                  role="button"
                  tabIndex={0}
                ...
                >
                  ...
                  <div className="p-2">
                    <p className="text-xs font-semibold text-white truncate">{fav.animeTitle}</p>
                    <p className="text-[10px] text-zinc-400 capitalize">{fav.status.replace("_", " ")}</p>
                  </div>
                </div>
              ))}
            </div>
